...
abstract class LoggerConfigurable {
	...
	protected function setInteger($property, $value) {
		try {
			$this->$property = \n2n\log4php\option\OptionConverter::toIntegerEx($value);
		} catch (\Exception $ex) {
			$value = var_export($value, true);
	...
	protected function setPositiveInteger($property, $value) {
		try {
			$this->$property = \n2n\log4php\option\OptionConverter::toPositiveIntegerEx($value);
		} catch (\Exception $ex) {
			$value = var_export($value, true);
	...
	protected function warn($message) {
		...
		throw new \n2n\log4php\LoggerException("log4php: $class: $message", E_USER_WARNING);
	}
}
