...
class OneToOneSelectCriteriaConstraint implements CriteriaConstraint {
	...
	function applyToCriteria(Criteria $criteria, CriteriaProperty $alias) {
		...
		$subCriteria->select($srcAlias)
				...
				->where()
						...
						->andMatch(CrIt::p($srcAlias, $this->srcEntityProperty), '=', $alias);
		...
		$subCriteria->select($srcAlias)
				->from($srcClass, $srcAlias)
				->where()->match(CrIt::p($srcAlias, $this->srcEntityProperty), '=', $alias);
