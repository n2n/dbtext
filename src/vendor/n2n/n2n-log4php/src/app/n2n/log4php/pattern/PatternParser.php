 ...
 * a chained list of {@link \n2n\log4php\pattern\PatternConverter} converters.</p>
 * 
...
class PatternParser {
	...
	public function parse() {
		...
		if ($count === false) {
			...
			throw new \n2n\log4php\LoggerException("Failed parsing layotut pattern: {$error['message']}");
		}
	 ...
	 * @param \n2n\log4php\formatting\FormattingInfo $info Formatting info.
	...
	private function getConverter($word, $info, $option) {
		if (!isset($this->converterMap[$word])) {
			throw new \n2n\log4php\LoggerException("Invalid keyword '%$word' in converison pattern. Ignoring keyword.");
		}
		
		$converterClass = $this->converterMap[$word];
		if(!class_exists($converterClass)) {
			throw new \n2n\log4php\LoggerException("Class '$converterClass' does not exist.");
		}
	...
	private function addToChain(\n2n\log4php\pattern\PatternConverter $converter) {
		if (!isset($this->head)) {
			$this->head = $converter;
			$this->tail = $this->head;
		} else {
	 ...
	 * \n2n\log4php\formatting\FormattingInfo object.
	 * 
	...
	private function parseModifiers($modifiers) {
		...
		if (!preg_match($pattern, $modifiers)) {
			throw new \n2n\log4php\LoggerException("log4php: Invalid modifier in conversion pattern: [$modifiers]. Ignoring modifier.", E_USER_WARNING);
			return $info;
		}
